...
import { Action, Spec } from './foInterface';
...
export class foStencilLibrary extends foLibrary {
  ...
  public find(id: string): foKnowledge {
    let found = this.concepts && this.concepts.getItem(id);
    if ( !found ) {
      found = this.factories && this.factories.getItem(id);
    }
    return found;
  }

  public creationFunction(type: string): () => any {
    let found = this.find(type);
    return (properties?: any, subcomponents?: any, parent?: any) => {
      return found && found.newInstance();
    };
  }

  public findConcept<T extends foNode>(id: string): foConcept<T> {
  ...
  public findFactory<T extends foNode>(id: string): foFactory<T> {
    const factory = this.factories && this.factories.getItem(id) as foFactory<T>;
    return factory;
  }
}
